...
int findVirus(int n,vector<int>&arr)
{
    while(arr.size()!=1)
    {
        int sum=0;
        ...
        int groupValue=sum/2;
        int nearestIndex=0;
        for(int i=0;i<arr.size();i++)
        {
            if(abs(arr[i]-groupValue)<abs(arr[nearestIndex]-groupValue))
            {
                nearestIndex=i;
            }
        }
        arr.erase(arr.begin()+nearestIndex);
    }
    return arr[0];
}
int main()
